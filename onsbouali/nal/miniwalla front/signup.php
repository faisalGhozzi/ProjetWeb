...
if($match > 0){
	...
	if(isset($_POST['firstName']) and isset($_POST['gov']) and isset($_POST['Num_Tel']) and isset($_POST['lastName']) and isset($_POST['password']) and isset($_POST['email']) and isset($_POST['adress']) and isset($_POST['gender']))
	{
	 ...
	 $status="Inactive";
		mysqli_query($conn,"insert into clients (first_name,last_name,password,email,Gouvernorat,adress,Num_Tel,gender,status) values('".$first_name."','".$last_name."','".$password."','".$mail."','".$gov."','".$adress."','".$tel."','".$gender."','".$status."')") or die(mysqli_error($conn));
